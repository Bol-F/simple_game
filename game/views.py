...
class CharacterCreationView(APIView):
    ...
    def post(self, request):
        ...
        agility = request.data.get("agility")
        endurance = request.data.get("endurance")

        logger.info(
            f"Class ID: {class_id}, Strength: {strength}, Agility: {agility}, Endurance: {endurance}"
        )
        ...
        try:
            ...
            character_data = {
                ...
                "health": character_class.health_per_level + int(endurance),
                "max_health": character_class.health_per_level + int(endurance),
                "total_level": 1,
                "battle_log": [],
            }
            ...
            logger.info(
                f"Game session {'created' if created else 'updated'}: {game_session}"
            )
...
class CurrentCharacterView(APIView):
    def get(self, request):
        logger.info(
            f"Fetching current character for session: {request.session.session_key}"
        )
        if not request.session.session_key:
            logger.error("No session key found")
            return Response({"error": "No character found"}, status=404)

        try:
            game_session = GameSession.objects.get(
                session_key=request.session.session_key
            )
            logger.info(f"Found game session: {game_session}")
            return Response(game_session.character_data)
        except GameSession.DoesNotExist:
            logger.error(
                f"No GameSession found for session key: {request.session.session_key}"
            )
            return Response({"error": "No character found"}, status=404)
...
class BattleView(APIView):
    ...
    def post(self, request):
        ...
        if battle_result["result"] == "win":
            ...
            battle_result["reward_weapon"] = WeaponSerializer(
                monster.reward_weapon
            ).data
        else:
